...
The Bitrae Core daemon can be configured to act as a trusted "border
router", implementing the bitrae wire protocol and relay, making
...
The ZeroMQ feature in Bitrae Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building bitraed:
...
For instance:
    ...
    $ bitraed -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/bitraed.tx.raw \
...
These options can also be provided in bitrae.conf.
...
From the perspective of bitraed, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into bitraed directly. Furthermore, no information is
...
using. Bitraed appends an up-counting sequence number to each
