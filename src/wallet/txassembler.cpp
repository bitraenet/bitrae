    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional BTRE outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes BTRE and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::BTRE_TO_BTRE) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        // First try to construct a BTRE-to-BTRE transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::BTRE_ONLY;
        ...
        // If BTRE-to-BTRE fails, create a peg-out transaction
